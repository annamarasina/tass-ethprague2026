...
import type { AuditInput, AuditLogEvent, AuditResult, EmitLog, Hex, SecurityReport, Severity } from "./interfaces";
...
async function runAudit(input: AuditInput, emit: EmitLog): Promise<AuditResult> {
...
async function runMockSecurityAnalysis(input: AuditInput, emit: EmitLog): Promise<SecurityReport> {
  ...
  const hasAssetHandling = /\b(deposit|withdraw|transfer|transferFrom|payable|vault|token)\b/i.test(source);
